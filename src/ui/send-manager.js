...
class SendManagerUI {
    ...
    async saveCurrentState() {
        try {
            ...
            const sendItems = this.history.filter(h => h.status !== 'cancelled');
    ...
    async checkAllFilesExistence() {
        ...
        for (const item of this.history) {
            if (item.mode !== 'send')
                continue;
            if (this.isReSharing[item.id])
                continue; // Skip if already re-sharing
            ...
            if (isFolder && snapshot.folderPath) {
                ...
                if (!currentContents.success) {
                    ...
                    if (!this.folderDeletedMap)
                        this.folderDeletedMap = {};
                        ...
                        try {
                            ...
                            if (!currentFiles || !currentFiles[filePath]) {
                                ...
                                if (this.fileHashCache)
                                    this.fileHashCache[filePath] = currentFiles[filePath].hash;
                            }
                            else {
                                if (this.fileExistsCache)
            ...
            if (changes.hasChanges) {
                ...
                if (changes.deleted.length > 0)
                    parts.push(`🗑️ ${changes.deleted.length} deleted`);
                if (changes.updated.length > 0)
                    parts.push(`✏️ ${changes.updated.length} updated`);
                if (changes.newFiles.length > 0)
                    parts.push(`➕ ${changes.newFiles.length} new`);
        ...
        try {
            ...
            if (snapshot.isFolder && snapshot.folderPath) {
                ...
                if (zipResult.success && zipResult.zipPath) {
                    ...
                    if (this.rezipErrors[itemId])
                        delete this.rezipErrors[itemId];
                }
                else {
    ...
    renderTable() {
        ...
        tbody.innerHTML = items.map((item) => {
            ...
            if (item.isTransferring && receiverList.length > 0) {
                receiversHtml = receiverList.map(([nodeId, data]) => {
