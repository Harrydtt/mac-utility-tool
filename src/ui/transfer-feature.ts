...
(function () {
    ...
    function switchTab(tab) {
        ...
        if (tab === 'send') {
            ...
            if (howSend) howSend.style.display = 'block';
            if (howReceive) howReceive.style.display = 'none';
        }
        else {
            ...
            if (howSend) howSend.style.display = 'none';
            if (howReceive) howReceive.style.display = 'block';
        }
    }
            ...
            for (const folderPath of paths) {
                ...
                if (transferState.zipMode === 'zip') {
                    ...
                    if (sizeRes.success && freeRes.success && sizeRes.sizeBytes > freeRes.freeBytes) {
                        ...
                        if (zipChk) zipChk.checked = false;
                        if (nozipChk) nozipChk.checked = true;
                    }
                }
    ...
    async function toggleZipMode(mode) {
        const zipChk = document.getElementById('tf-chk-zip') as HTMLInputElement;
        const nozipChk = document.getElementById('tf-chk-nozip') as HTMLInputElement;
        const warning = document.getElementById('tf-zip-warning');

        // If clicking the already active mode, prevent unchecking
        if (mode === transferState.zipMode) {
            if (mode === 'zip' && zipChk) zipChk.checked = true;
            if (mode === 'no-zip' && nozipChk) nozipChk.checked = true;
            return;
        }
        ...
        if (mode === 'zip') {
            if (zipChk) zipChk.checked = true;
            if (nozipChk) nozipChk.checked = false; // Uncheck the other
        } else {
            if (zipChk) zipChk.checked = false; // Uncheck the other
            if (nozipChk) nozipChk.checked = true;
        }
        ...
        for (const path of transferState.selectedFiles) {
            if (!transferState.itemMetadata) transferState.itemMetadata = {};
    ...
    function renderFileList() {
        ...
        if (transferState.selectedFiles.length > 0) {
            ...
            const zipChk = document.getElementById('tf-chk-zip');
            const nozipChk = document.getElementById('tf-chk-nozip');
            if (zipChk) zipChk.checked = transferState.zipMode === 'zip';
            if (nozipChk) nozipChk.checked = transferState.zipMode === 'no-zip';
        }
        else {
    ...
    async function proceedSend() {
        if (transferState.selectedFiles.length === 0) return;
    ...
    async function pollTransferStatus() {
        if (transferState.mode === 'receive') {
            ...
            if (pending) setTimeout(pollTransferStatus, 1000);
            return;
        }
