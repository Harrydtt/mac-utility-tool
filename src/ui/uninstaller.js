...
async function scanAppForUninstall(appPath) {
    ...
    try {
        ...
        // Check for Mac Ultility Tool (block) or Apple apps (warning)
        const appName = selectedApp.name.toLowerCase();
        const isMacCleaner = appName.includes('maccleaner') || appName.includes('mac cleaner') || appName.includes('mac ultility tool');
        const isBlocked = isBlockedSystemApp(appName);
        const isAppleApp = isAppleApplication(appName);

        // Handle Mac Ultility Tool - BLOCK
        if (isMacCleaner) {
            showMacCleanerWarning();
        }
        // Handle System Apps - BLOCK
        else if (isBlocked) {
            showSystemAppBlockWarning(selectedApp.name);
        }
...
// Apps that are part of the system (read-only volume) and CANNOT be uninstalled
const BLOCKED_SYSTEM_APPS = [
    'music', 'safari', 'photos', 'mail', 'messages', 'facetime',
    'stocks', 'news', 'tv', 'home', 'finder', 'system settings',
    'app store', 'podcasts', 'weather', 'clock', 'maps',
    'voice memos', 'calculator', 'calendar', 'contacts', 'notes',
    'reminders', 'textedit', 'preview', 'quicktime player', 'chess',
    'books', 'dictionary', 'font book', 'automator', 'siri', 'mission control',
    'launchpad', 'stickies', 'system preferences', 'photobooth', 'photo booth',
    'freeform'
];

// Apple apps that CAN be uninstalled (usually from App Store) but might be important
const WARNING_APPLE_APPS = [
    'xcode', 'pages', 'numbers', 'keynote', 'garageband', 'imovie',
    'final cut', 'logic pro', 'motion', 'compressor', 'mainstage',
    'configurator', 'support', 'classroom', 'swift playgrounds'
];

// Check if app is an Apple app
function isAppleApplication(appName) {
    const lowerName = appName.toLowerCase();
    // Only return true if it's in the warning list, blocked ones are handled separately
    return WARNING_APPLE_APPS.some(appleApp => lowerName.includes(appleApp));
}

// Check if app is a BLOCKED system app
function isBlockedSystemApp(appName) {
    const lowerName = appName.toLowerCase();
    // Check for exact match or strict container match
    // E.g. "Music" matches, "Music Player" (3rd party) should not. 
    // But "Apple Music" should match. 
    // Let's use strict includes for safety, as most system apps have simple names.
    return BLOCKED_SYSTEM_APPS.some(sysApp => lowerName === sysApp || lowerName === `apple ${sysApp}`);
}
    ...
    warningContainer.innerHTML = `
        <div class="uninstall-warning block-warning" style="background: rgba(239, 68, 68, 0.15); border: 1px solid #ef4444; border-radius: 8px; padding: 1rem; margin-bottom: 1rem;">
            <div style="display: flex; align-items: center; gap: 0.75rem;">
                ...
                <div>
                    <strong style="color: #ef4444;">Cannot Uninstall Mac Ultility Tool</strong>
                    <p style="color: var(--text-secondary); margin: 0.25rem 0 0 0; font-size: 0.85rem;">
                        Mac Ultility Tool cannot be used to clean itself.<br>
                        Please use Finder or another app to uninstall Mac Ultility Tool.
                    </p>
                </div>
            </div>
        </div>
...
// Show System App BLOCK warning (Read-only volume)
function showSystemAppBlockWarning(appName) {
    const uninstallBtn = document.querySelector('.uninstall-actions .danger-btn');
    const warningContainer = getOrCreateWarningContainer();
    ...
    if (uninstallBtn) {
        ...
        uninstallBtn.style.opacity = '0.5';
        uninstallBtn.style.cursor = 'not-allowed';
        uninstallBtn.title = 'Cannot uninstall system application';
    }
    ...
    warningContainer.innerHTML = `
        <div class="uninstall-warning block-warning" style="background: rgba(239, 68, 68, 0.15); border: 1px solid #ef4444; border-radius: 8px; padding: 1rem; margin-bottom: 1rem;">
            <div style="display: flex; align-items: center; gap: 0.75rem;">
                ...
                <div>
                    <strong style="color: #ef4444;">Protected System App</strong>
                    <p style="color: var(--text-secondary); margin: 0.25rem 0 0 0; font-size: 0.85rem;">
                        <strong>${appName}</strong> is a core macOS application located on a read-only volume.<br>
                        It cannot be modified or deleted by 3rd party apps.
                    </p>
                </div>
            </div>
        </div>
...
// Show Apple app warning (Removable but risky)
