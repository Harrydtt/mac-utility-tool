...
export async function cleanCommand(options: CleanCommandOptions): Promise<CleanSummary | null> {
  ...
  for (const { categoryId, items } of selectedItems) {
    ...
    let result: { success: boolean; error?: string; successCount?: number; failedCount?: number };

    if (options.dryRun) {
      result = { success: true, successCount: items.length };
    } else {
    ...
    if (result.successCount > 0) {
      // Estimate freed space based on success count vs total
      // This is an approximation if we don't know which specific files failed,
      // but usually items have similar size distribution or we accept the error margin.
      // Better: we can't easily know EXACT space of successful items without tracking per file return.
      // For CLI summary, proportional size is acceptable or we assume full size minus error.
      // Let's rely on successCount.

      const ratio = items.length > 0 ? result.successCount / items.length : 0;
      const totalSize = items.reduce((sum, i) => sum + i.size, 0);
      const freedSpace = Math.floor(totalSize * ratio);

      cleanResults.results.push({
        category: scanner.category,
        cleanedItems: result.successCount,
        freedSpace,
        errors: result.error ? [result.error] : [],
      });
      cleanResults.totalFreedSpace += freedSpace;
      cleanResults.totalCleanedItems += result.successCount;
    } else {
      cleanResults.results.push({
        category: scanner.category,
        cleanedItems: 0,
