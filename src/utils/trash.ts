...
export async function moveToTrash(paths: string[]): Promise<{ success: boolean; error?: string; successCount?: number; failedCount?: number }> {
  ...
  try {
    ...
    const failedCount = failedItems.length + permissionDenied;
    console.log(`[moveToTrash] Complete: ${successCount} moved, ${permissionDenied} skipped (permission denied), ${failedItems.length} failed`);

    // Return success if we moved at least some files
    return {
      success: successCount > 0 || paths.length === permissionDenied,
      successCount,
      failedCount,
      error: failedItems.length > 0 ? `${failedItems.length} items failed: ${failedItems.slice(0, 2).join(', ')}${failedItems.length > 2 ? '...' : ''}` : undefined
    };
  } catch (error: any) {
