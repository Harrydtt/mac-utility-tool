...
import { interactiveCommand, listCategories, maintenanceCommand, uninstallCommand, uiCommand, transferFilesCommand } from './commands/index.js';
...
program
  .command('ui')
  .description('Launch the Web Dashboard UI')
...
program
  .command('transfer')
  .description('Transfer files P2P using AltSendme (auto-updates from GitHub)')
  .option('-u, --update', 'Force update to latest version from GitHub')
  .action(async (options) => {
    try {
      await transferFilesCommand({
