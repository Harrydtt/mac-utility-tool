...
// Scan for threats
export async function scanThreats(scanPaths: string[], workerCount: number = 2): Promise<DetectedThreat[]> {
    ...
    // ClamAV Deep Scan (if installed) - using parallel workers
    try {
        const { checkClamAVInstalled, scanWithClamAVParallel } = await import('../utils/clamav.js');
        const { installed } = await checkClamAVInstalled();

        if (installed) {
            console.log('[Threats] Running ClamAV parallel deep scan with 2 workers...');
            ...
            if (existingPaths.length > 0) {
                // Use parallel workers based on user selection
                const clamResult = await scanWithClamAVParallel(existingPaths, workerCount);
