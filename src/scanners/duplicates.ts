...
export class DuplicatesScanner extends BaseScanner {
  ...
  async scan(options?: ScannerOptions): Promise<ScanResult> {
    ...
    for (const searchPath of DEFAULT_SEARCH_PATHS) {
      if (await exists(searchPath)) {
        await this.collectFiles(searchPath, filesBySize, minSize, MAX_DEPTH, options);
      }
    }
  ...
  private async collectFiles(
    ...
    // Check ignored folders (prefix match)
    if (options?.ignoredFolders && options.ignoredFolders.some(f => normalizedDir === f || normalizedDir.startsWith(f + '/'))) {
      return;
    }
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        // Check ignored paths (exact match)
        if (options?.ignoredPaths && options.ignoredPaths.has(normalizedPath)) {
          continue;
        }
        ...
        try {
          if (entry.isFile()) {
            ...
            if (options?.ignoredFolders && options.ignoredFolders.some(f => normalizedPath === f || normalizedPath.startsWith(f + '/'))) {
              continue;
            }
            await this.collectFiles(fullPath, filesBySize, minSize, maxDepth, options, currentDepth + 1);
          }
        } catch {
          continue;
        }
